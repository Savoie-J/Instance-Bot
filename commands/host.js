...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      // Handle 'restrict' subcommand (from restrict.js)
      if (interaction.options.getSubcommand() === "restrict") {
        // Delegate to the restrict command handler
      ...
      if (embedFunction) {
        ...
        const createUpdatedEmbed = (currentEmbed) => {
          ...
          if (
            addFill &&
            !embed.data.fields.some((field) => field.name === ":scales: Fill")
          ) {
            embed.addFields({
              name: ":scales: Fill",
              value: "`Empty`",
          ...
          embed.data.fields.forEach((field) => {
            if (field.value !== "`Empty`") {
              field.value.split(", ").forEach((user) => uniqueUsers.add(user));
            }
          });
        ...
        collector.on("collect", async (i) => {
          ...
          if (i.customId === "complete") {
            ...
            if (!i.member.permissions.has(PermissionFlagsBits.ManageEvents)) {
              await i.reply({
                content:
            ...
            if (currentEmbed && currentEmbed.fields) {
              ...
              if (!uniqueUsers.has(`<@${i.user.id}>`)) {
                await i.reply({
                  content: "Only those who are signed up may notify players.",
                  ephemeral: true,
                });
                return;
              }
              ...
              const notificationMessage = Array.from(uniqueUsers).join("\n");
            ...
            if (fieldIndex !== -1) {
              ...
              const userFields = fields.filter((f) => f.value.includes(userId));
              const currentEmbed = i.message.embeds[0];
              ...
              const { embed: updatedEmbed, actionRows } = createUpdatedEmbed({
                ...currentEmbed,
                fields: fields,
              });
              ...
              await i.reply({ content: "Field not found", ephemeral: true });
            }
          }
        });
